...
class Console():
    ...
    def printInfo(text: str, _type: PrintType, margin=0, date=False):
        ...
        for i in range(margin):
            _margin += " "
        if date:
            print(_margin + f"{Colors.cyan}[{Colors.gray}{datetime.now().strftime('%H:%M:%S')}{Colors.cyan}]" + str(_type).replace("%status%", Colors.yellow + "!").replace("%text%", text))
        else:
            print(_margin + str(_type).replace("%status%", Colors.yellow + "!").replace("%text%", text))
    
    def input(text: str, _type: PrintType, margin=0, date=False):
        _margin = ""
        for i in range(margin):
            _margin += " "
        if date:
            return input(_margin + f"{Colors.cyan}[{Colors.gray}{datetime.now().strftime('%H:%M:%S')}{Colors.cyan}]" + str(_type).replace("%status%", Colors.yellow + "?").replace("%text%", text) + ">")
        else:
            return input(_margin + str(_type).replace("%status%", Colors.yellow + "?").replace("%text%", text + ">"))
...
class RefreshingScreen:
    def __init__(self, banner: str, ln1: str, ln2: str, ln3="", ln4="", ln5="", refreshInterval=0.2, bannerColor=Colors.red, ligneColors=Colors.light_red, screenType=RefreshingScreenType.CLASSIC):
        self.banner = banner
        ...
        self.ligneColors = ligneColors
        self.screenType = screenType

        self.thread = threading.Thread(target=self.refreshScreenLoop)
    ...
    def updateVal5(self, val):
        self.val5 = val

    def updateValue(self, val1=None,val2=None,val3=None,val4=None,val5=None):
        if val1 != None:
            self.val1 = val1
        if val2 != None:
            self.val2 = val2
    ...
    def printScreen(self):
        ...
        screenType = screenType.replace("%ln1%", Center.XCenter(f"{self.ligneColors}{self.ln1}: {self.val1}"))
        screenType = screenType.replace("%ln2%", Center.XCenter(f"{self.ligneColors}{self.ln2}: {self.val2}"))
        if self.ln3 == None:
            screenType = screenType.replace("%ln3%", f"")
        else:
            screenType = screenType.replace("%ln3%", Center.XCenter(f"{self.ligneColors}{self.ln3}: {self.val3}"))
        if self.ln4 == None:
            screenType = screenType.replace("%ln4%", f"")
        else:
            screenType = screenType.replace("%ln4%", Center.XCenter(f"{self.ligneColors}{self.ln4}: {self.val4}"))
        if self.ln5 == None:
            screenType = screenType.replace("%ln5%", f"")
        else:
